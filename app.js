...
const gameBoard = (function(){
    ...
    const startGame = () => {
        ...
        const squaresArr = Array.from(boardSquares);
                ...
                if (isPlaying && boardArr[index] === "")
                {
                    ...
                    else if(boardArr.filter(elem => elem).length === 9)
                    {
                        ...
                        if(player2.isCPU())
                        {
                            isPlaying = false;
                            setTimeout(() => {
                                fillSquare(cpuPlayer.getRobotMoveIndex(boardArr));
                                //Check for Draw
                                if(checkWinCondition())
                                {
                                    //Win
    ...
    const endGame = () => {
        // //Win msg
        // let winBtn = document.querySelector('#winner-btn');
        // winBtn.style.cssText = "display: show; animation:popup 0.3s ease forwards;";
        // winBtn.addEventListener("click", e => {
        //     displayMsg();
        //     winBtn.style.cssText = "animation: fade-out 0.3s ease forwards";
    ...
    const restart = () => {
        ...
        if(player1.getFlags() < maximumFlags && player2.getFlags() < maximumFlags)
        {
            ...
            else
            {
                ...
                //Restarts game on the next player turn
            }
        }
        else
        {
    ...
    const fillSquare = index => {
        ...
        boardSquares[index].appendChild(spanText);
    }
    ...
    // const displayMsg = () => { //----------ATACH THIS TO ENDGAME() ----> Will Revisit it later
    //     let header = document.querySelector('h1');
    
    //     let changeTxtHeader = header.innerHTML = "Green player has won the game" //Add dynamic Player msg here
    //     if (player1 === maximumFlags) {
    //         return changeTxtHeader
    //     }
    // }
...
const factoryPlayer = (name, sideID, type) =>{
    const playerIcon = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-person" viewBox="0 0 16 16">
    <path fill-rule="evenodd" d="M10 5a2 2 0 1 1-4 0 2 2 0 0 1 4 0zM8 8a3 3 0 1 0 0-6 3 3 0 0 0 0 6zm6 5c0 1-1 1-1 1H3s-1 0-1-1 1-4 6-4 6 3 6 4zm-1-.004c-.001-.246-.154-.986-.832-1.664C11.516 10.68 10.289 10 8 10c-2.29 0-3.516.68-4.168 1.332-.678.678-.83 1.418-.832 1.664h10z"/>
    </svg>`;

    const cpuIcon = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-cpu" viewBox="0 0 16 16">
    ...
    const flagIcon = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-flag-fill" viewBox="0 0 16 16">
    ...
    type === "player"? ICON.innerHTML = playerIcon: ICON.innerHTML = cpuIcon;

    const playerFlags = playerSide.querySelector(".player-wins");
    ...
    const addFlag = () => {
        playerFlags.innerHTML += flagIcon;
        flags ++;
    }
